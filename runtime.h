...
class Runtime{
	...
	friend std::ostream& operator <<(std::ostream& outs, const Runtime& rhs);
	friend std::istream& operator >>(std::istream& ins, Runtime& rhs);
	friend Runtime operator *(const double& lhs, const Runtime& rhs);
